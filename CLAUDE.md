...
They are deployed and run independently (backend on Railway - `henjo-backend/railway.json` - frontend on Vercel, which ignores `henjo-backend/` via the root `.vercelignore`); there is no shared build tooling between them. The backend's Blade/Vite setup (`resources/views/welcome.blade.php`, `vite.config.js`) is unused boilerplate - `routes/web.php` only serves the default Laravel welcome view. All real frontend work happens in the Next.js app.
...
- **Public JSON API** - `routes/api.php`, all under `/api/v1/...`, handled by controllers in `app/Http/Controllers/Api/`. Read-only GET endpoints for safaris, destinations, posts, team members, activities, plus the site-content endpoints below; POST endpoints for inquiries and bookings. Every response is shaped `{ success: bool, data, message?, errors? }`. List endpoints (`SafariPackageController::index`) support query-string filtering (`search`, `category`, `destination`, `activity`, `country`) and are paginated (`per_page`, default 12) via Eloquent's paginator, so `data` is a Laravel pagination object, not a bare array. The `/posts/*` routes and `PostController`/Filament `PostResource` are still fully live on the backend, but the frontend blog UI that consumed them was removed (see Frontend section) - treat blog as an API-only, admin-manageable feature with no public page right now.
...
- CORS (`config/cors.php`) allows `localhost:3000`/`127.0.0.1:3000` plus any origins in the `CORS_ALLOWED_ORIGINS` env var, and matches `allowed_origins_patterns` against `https://*.vercel.app` so preview/demo deploys work without hardcoding a URL - tighten the pattern to the real production domain once one is finalized.

### Frontend: Next.js App Router talking to the API through one client

- Routes live under `app/<route>/page.tsx` (e.g. `app/safaris/[slug]/page.tsx`, `app/booking/page.tsx`, `app/about-our-charity/page.tsx`). Components are grouped by feature under `components/<feature>/` (`components/safari/`, `components/destination/`, `components/common/`). The blog frontend (`app/blog/`, `components/blog/`, `lib/api/blogApi.ts`, `types/blog.ts`) was removed entirely - the backend blog API is untouched (see Backend section), there's just no page consuming it anymore.
- All HTTP calls go through the singleton `apiClient` in `lib/api/client.ts` (axios wrapper, base URL from `NEXT_PUBLIC_API_URL`, adds a bearer token from `localStorage` if present, unwraps the backend's `{success, data}` envelope). Feature-specific request functions live in `lib/api/<feature>Api.ts` (`safariApi.ts`, `bookingApi.ts`, `contactApi.ts`, `teamApi.ts`, `menuApi.ts`, `pagesApi.ts`, `settingsApi.ts`, `destinationApi.ts`) and wrap `apiClient.get/post/...` - add new endpoints there rather than calling `apiClient` directly from components.
- Types in `types/*.ts` (`safari.ts`, `team.ts`, `api.ts`, `menu.ts`, `page.ts`, `settings.ts`) mirror the backend's Eloquent model shapes (including the `media` array and pagination envelope) - keep them in sync when backend fields change.
...
Earlier history: schema/admin dashboard were rebuilt from scratch, then a full booking flow shipped (`BookingController`, `NewBookingNotification` mail, a migration making `bookings.package_id` nullable so a booking isn't required to name a specific package, the admin `Bookings` resource/table showing customer + package names instead of raw IDs, and the frontend `app/booking/` page with country -> package cascading selects plus a working "Book Now" modal on the safari detail page). The site-content CMS layer (`SiteSetting`, `Menu`, `Page` models + their API endpoints + Filament resources/`SettingsPage`) was then added, `app/layout.tsx` switched to server-fetch settings/menus, and several previously-hardcoded frontend pages were rewired to pull their content from it; a `ReportsPage` with CSV-exportable analytics widgets was added alongside the existing `Dashboard`. The homepage hero's background video now reads `settings?.homepage_hero_url` first, falling back to the hardcoded `HERO_VIDEO_URL` storage path in `app/page.tsx` only if `SiteSetting::homepage_hero_url` isn't set - the earlier drift between the two is reconciled.

Most recently, the project moved from placeholder/demo content to real client content and got a first production-style deploy (Railway for the backend, Vercel for the frontend):

- **Real safari package data**: all ~38 placeholder packages were replaced with 80 fully-detailed real packages (20 per country), parsed from client-supplied SafariBookings.com audit docs and seeded via `RealSafariPackageSeeder` (`database/seeders/data/real_safari_packages.json`). New package/itinerary schema fields support this: price ranges, min age, trip privacy, comfort level, accommodation style, customizable/solo-friendly flags, and start flexibility on packages; destination/accommodation/day-span fields on itinerary days. The package detail page (`app/safaris/[slug]/page.tsx`) was redesigned around this data (price ranges, trip-style badges, a "Good to Know" strip, per-day destination/accommodation).
- **Activities taxonomy** expanded from 8 to 18 canonical activities, with a new `featured` flag admins toggle to control which ones surface on the homepage's "Popular Experiences" section (renamed from "Activities").
- **Homepage**: activities and testimonials are now auto-scrolling marquee carousels; a real Partners section (client-supplied logos) was added; the "Ready for Your African Adventure?" CTA uses a looping video background instead of a static photo.
- **Client-requested content/UX changes**: WhatsApp contact wired in (floating button, footer, safari detail page - `lib/utils/whatsapp.ts`), Pesapal payment link made prominent (footer, safari detail sidebar), TripAdvisor link and social handles added, a real "Our Founder" section on the About page, an expanded Charity page with a new `app/about-our-charity/` route, Kenya/Tanzania-first destination ordering, and copy cleanup site-wide (no em-dashes, "Bespoke" -> "custom-designed"/"custom-tailored"). The real logo (`henjo_icon_logo.webp`, attached to `SiteSetting` via `PageContentSeeder`) replaced the placeholder navbar/footer logo, and the "Henjo African Safaris" text label was dropped from the navbar (logo mark only).
- **Blog frontend removed** (`app/blog/`, `components/blog/`, `lib/api/blogApi.ts`, `types/blog.ts` all deleted) - the client wanted the Travel Information page to host visa-guide content directly instead of linking to blog posts. The backend blog API/admin resource is untouched and still functional, just unused by the frontend now.
- Known gap: Kenya/US contact placeholders were removed pending new details from the client - no real Kenya/US contact info is wired in yet.

`HENJOSAFARIS_LIVE_CONTENT.md` at the repo root holds real copy/content pulled from the live site, used to seed the above (see `PageContentSeeder`, `DestinationSeeder`, `RealSafariPackageSeeder`).
...
- Characters: keyboard-only ASCII everywhere (code, comments, copy, commit messages, docs). No em/en dashes (use `-` or reword), no curly quotes, ellipsis, arrows, bullets, or their HTML entities. No emojis anywhere - use an icon component instead (Heroicon in Filament/Blade, `react-icons` in the frontend). Currency symbols and diacritics in real proper names are the only allowed exceptions.
